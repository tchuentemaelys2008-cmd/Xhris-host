...
export default function LoginPage() {
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ background: 'radial-gradient(ellipse at 50% 50%, #0D0A1F 0%, #080810 100%)' }}>
      ...
      {/* Card */}
      <div className="relative z-10 flex-1 flex items-center justify-center px-4 -mt-12">
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="w-full max-w-[440px]">
          <div className="bg-[#0E0E1C]/95 border border-white/8 rounded-2xl px-10 py-10 shadow-2xl">
            ...
            <h1 className="text-2xl font-bold text-white text-center mb-2">Bienvenue de retour</h1>
            <p className="text-gray-400 text-sm text-center leading-relaxed mb-8">
              Connectez-vous à votre compte XhrisHost pour<br />gérer vos bots et serveurs.
            </p>

            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className="text-sm text-gray-300 block mb-1.5">Email ou nom d'utilisateur</label>
                <div className="relative">
                  <Mail className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-500" />
                  <input
                    type="text" value={email} onChange={e => setEmail(e.target.value)}
                    placeholder="Entrez votre email ou nom d'utilisateur"
                    className="w-full bg-white/5 border border-white/10 rounded-xl pl-11 pr-4 py-3.5 text-white text-sm placeholder-gray-600 focus:outline-none focus:border-purple-500/50 focus:bg-white/8 transition-all"
                    required
                  />
                </div>
              </div>
              ...
              <div>
                <label className="text-sm text-gray-300 block mb-1.5">Mot de passe</label>
                <div className="relative">
                  <Lock className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-500" />
                  <input
                    type={showPassword ? 'text' : 'password'} value={password} onChange={e => setPassword(e.target.value)}
                    placeholder="Entrez votre mot de passe"
                    className="w-full bg-white/5 border border-white/10 rounded-xl pl-11 pr-12 py-3.5 text-white text-sm placeholder-gray-600 focus:outline-none focus:border-purple-500/50 transition-all"
                    required
                  />
              ...
              <label className="flex items-center gap-3 cursor-pointer group">
                <div
                  ...
                  className={`w-5 h-5 rounded flex items-center justify-center flex-shrink-0 border transition-all ${remember ? 'bg-purple-600 border-purple-600' : 'bg-transparent border-white/20 group-hover:border-white/40'}`}
                >
                  {remember && <svg className="w-3 h-3 text-white" fill="none" viewBox="0 0 12 12"><path d="M2 6l3 3 5-5" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg>}
                </div>
              ...
              <button
                ...
                className="w-full bg-purple-600 hover:bg-purple-500 text-white rounded-xl py-3.5 font-semibold text-sm flex items-center justify-center gap-2 transition-all active:scale-[0.99] mt-1 disabled:opacity-60"
              >
                {loading
            ...
            <button
              ...
              className="w-full bg-white hover:bg-gray-100 text-gray-800 rounded-xl py-3.5 font-medium text-sm flex items-center justify-center gap-3 transition-all active:scale-[0.99]"
            >
            ...
            <p className="text-center text-sm text-gray-500 mt-6">
              ...
              <Link href="/auth/register" className="text-purple-400 hover:text-purple-300 font-medium">Créer un compte</Link>
            </p>
          </div>
        </motion.div>
      </div>
