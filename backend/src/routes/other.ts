...
// ════════════════════════════════════════════════════════════════════
// TESTIMONIES — Témoignages utilisateurs
// ════════════════════════════════════════════════════════════════════
...
// POST /api/support/testimony - Soumettre un témoignage (user via JWT OU bot via x-api-key)
supportRouter.post('/testimony', async (req: any, res: Response) => {
  try {
    const auth = await resolveTestimonyUser(req);
    if (!auth) return sendError(res, 'Auth requise', 401);
    ...
    if (trimmed.length < 10) return sendError(res, 'Temoignage trop court (min 10 caracteres)', 400);
    if (trimmed.length > 1000) return sendError(res, 'Temoignage trop long (max 1000 caracteres)', 400);

    const ratingNum = Number(rating);
    const safeRating = (!isNaN(ratingNum) && ratingNum >= 1 && ratingNum <= 5) ? Math.round(ratingNum) : 5;

    // Anti-spam: 1 témoignage par jour par user
    const todayStart = new Date();
    todayStart.setHours(0, 0, 0, 0);
    const existing = await prisma.testimony.findFirst({
      where: { userId: auth.userId, createdAt: { gte: todayStart } },
    });
    if (existing) return sendError(res, 'Vous avez deja soumis un temoignage aujourd\'hui', 429);
    ...
    // Récompense : +5 coins
    await prisma.$transaction([
      prisma.user.update({ where: { id: auth.userId }, data: { coins: { increment: 5 } } }),
      prisma.transaction.create({
        data: {
          userId: auth.userId,
          type: 'BONUS_CODE' as any,
          amount: 5,
    ...
    sendSuccess(res, { id: testimony.id }, 'Temoignage soumis. Il apparaitra apres validation.');
  } catch (err: any) {
    sendError(res, 'Erreur lors de la soumission', 500);
  }
});

// GET /api/support/testimony/public - Liste publique des témoignages approuvés (pas d'auth)
supportRouter.get('/testimony/public', async (_req: any, res: Response) => {
  try {
...
// GET /api/support/testimony/mine - Mes témoignages (JWT ou x-api-key)
supportRouter.get('/testimony/mine', async (req: any, res: Response) => {
  try {
    const auth = await resolveTestimonyUser(req);
    if (!auth) return sendError(res, 'Auth requise', 401);
...
// GET /api/support/testimony/admin - Tous les témoignages (admin uniquement)
supportRouter.get('/testimony/admin', async (req: any, res: Response) => {
  try {
    const auth = await resolveTestimonyUser(req);
    if (!auth?.isAdmin) return sendError(res, 'Acces admin requis', 403);
    const list = await prisma.testimony.findMany({
...
// PATCH /api/support/testimony/:id - Modérer (admin uniquement)
supportRouter.patch('/testimony/:id', async (req: any, res: Response) => {
  try {
    const auth = await resolveTestimonyUser(req);
    if (!auth?.isAdmin) return sendError(res, 'Acces admin requis', 403);
    ...
    if (!Object.keys(data).length) return sendError(res, 'Aucun changement', 400);

    const t = await prisma.testimony.update({ where: { id: req.params.id }, data });
    sendSuccess(res, t, 'Temoignage mis a jour');
  } catch (err: any) {
    sendError(res, 'Erreur: ' + (err?.message || 'inconnue'), 500);
  }
});

// DELETE /api/support/testimony/:id - Auteur ou admin
supportRouter.delete('/testimony/:id', async (req: any, res: Response) => {
  try {
    const auth = await resolveTestimonyUser(req);
    if (!auth) return sendError(res, 'Auth requise', 401);

    const t = await prisma.testimony.findUnique({ where: { id: req.params.id } });
    if (!t) return sendError(res, 'Temoignage introuvable', 404);

    if (t.userId !== auth.userId && !auth.isAdmin) {
      return sendError(res, 'Permission refusee', 403);
    }
    await prisma.testimony.delete({ where: { id: req.params.id } });
    sendSuccess(res, null, 'Temoignage supprime');
  } catch {
    sendError(res, 'Erreur', 500);
  }
});
