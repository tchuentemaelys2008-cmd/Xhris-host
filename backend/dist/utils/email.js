...
async function sendEmail(to, subject, html) {
    ...
    if (!apiKey) {
        logger_1.logger.info(`[Email skipped - no RESEND_API_KEY] To: ${to} | Subject: ${subject}`);
        return;
    }
    try {
        ...
        logger_1.logger.error('Resend email error:', err);
        throw err;
    }
}
...
async function sendVerificationEmail(to, name, token) {
    ...
    await sendEmail(to, 'Vérifiez votre email — XHRIS HOST', baseTemplate(`Bienvenue, ${name} !`, para('Cliquez sur le bouton ci-dessous pour vérifier votre adresse email et activer votre compte.') +
...
async function sendPasswordResetEmail(to, name, token) {
    ...
    await sendEmail(to, 'Réinitialisation de mot de passe — XHRIS HOST', baseTemplate(`Réinitialisation de mot de passe`, para(`Bonjour ${name},`) +
...
async function sendWelcomeEmail(to, name) {
    await sendEmail(to, 'Bienvenue sur XHRIS HOST ! ⚡', baseTemplate(`Bienvenue ${name} !`, para('Votre compte a été créé avec succès. Vous avez reçu <strong style="color:#fbbf24;">10 coins de bienvenue</strong> pour démarrer.') +
...
async function sendOtpEmail(to, name, otp) {
    await sendEmail(to, 'Code de vérification — XHRIS HOST', baseTemplate('Votre code OTP', para(`Bonjour ${name},`) +
...
async function sendDeploymentConfirmationEmail(to, name, botName) {
    await sendEmail(to, `Bot déployé : ${botName} — XHRIS HOST`, baseTemplate('Déploiement réussi !', para(`Bonjour ${name},`) +
...
async function sendServerNotificationEmail(to, name, serverName, message) {
    await sendEmail(to, `Alerte serveur : ${serverName} — XHRIS HOST`, baseTemplate(`Notification serveur`, para(`Bonjour ${name},`) +
...
async function sendSubscriptionInvoiceEmail(to, name, plan, amount, period) {
    await sendEmail(to, `Facture abonnement ${plan} — XHRIS HOST`, baseTemplate('Votre facture', para(`Bonjour ${name},`) +
...
async function sendServerExpirationAlertEmail(to, name, serverName, daysLeft) {
    ...
    await sendEmail(to, `${isUrgent ? '🚨 ' : ''}Expiration serveur : ${serverName} — XHRIS HOST`, baseTemplate(isUrgent ? 'Action requise — Serveur bientôt expiré' : `Rappel : expiration dans ${daysLeft} jours`, para(`Bonjour ${name},`) +
